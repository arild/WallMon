...
STouchManager::STouchManager(void *refcon, touchManagerCallback_t callback, uint32_t wantedSenderID) : refcon(refcon), touchesDown(), touchesUp(), wantedSenderID(wantedSenderID), callback(callback) {
...
void		STouchManager::handleEvent(shout_event_t *evt) {
	...
	if (evt->type == kEvt_type_calibrated_touch_location) {
		bool				isLast			= false;
		TT_touch_state_t	*state			= newTouchStateForEvent(evt, isLast),
							*existingState	= 0;
		if (state) {
		...
		if (parse_touch_remove_event(evt, &oid, &senderID) == 0) {
			if (senderID == (uint32_t)wantedSenderID || wantedSenderID == -1) {
				...
				if (map_it != oidTouchMap.end()) {
					...
					for (touchVector_t::iterator vec_it=touchesDown.begin();vec_it != touchesDown.end(); ++vec_it) {
						if (*vec_it == state) {
...
void	STouchManager::lock(void) { pthread_mutex_lock(&mgrLock); }
void	STouchManager::unlock(void) { pthread_mutex_unlock(&mgrLock); }
